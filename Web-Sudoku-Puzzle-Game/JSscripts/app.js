    ...
    document.addEventListener('keydown', function(event) {
        if (!disableSelect) {
            ...
            if (event.key >= '1' && event.key <= '9') {
                ...
                let selectedTile = qs(".tile.selected");
                if (selectedTile) {
                    // Set the selected tile's text to the pressed key
                    selectedTile.textContent = event.key;
...
function updateMove() {
    let selectedTile = qs(".tile.selected");
    if (selectedTile && selectedTile.textContent !== "") {
